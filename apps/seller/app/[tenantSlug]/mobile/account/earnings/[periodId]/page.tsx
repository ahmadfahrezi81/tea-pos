...
export default function PayslipPage({ params }: { params: Promise<{ periodId: string }> }) {
    ...
    const entriesByDate = entries.reduce<Record<string, typeof entries[0][]>>((acc, e) => {
        ...
        <div className="space-y-3 pb-4">
            <div className="bg-white rounded-xl p-4 space-y-1 font-mono">
                ...
                {approvedClaims.length > 0 && (
                    <>
                        ...
                        {approvedClaims.map((r) => (
                            <Row
                                key={r.id}
                                left={REIMBURSEMENT_TYPE_LABELS[r.type as keyof typeof REIMBURSEMENT_TYPE_LABELS] ?? r.type}
                                right={`Rp ${r.amount.toLocaleString("id-ID")}`}
                            />
                        ))}
                        <Divider />
                    </>
                ...
                <div className="pt-3 text-center space-y-2">
                    <p className={`text-sm font-bold tracking-wide font-mono ${STATUS_COLORS[status] ?? "text-gray-500"}`}>
                    ...
                    {!hasBankInfo && status !== "paid" && (
                        <p className="text-xs text-amber-600">Add your bank details in Personal Details so admin can pay you.</p>
                    )}
                </div>
