...
export default function EarningsPage() {
    ...
    const { ratePerCup, effectiveDate, isLoading: rateLoading } = useCommissionConfig(user?.id);

    const isLoading = periodsLoading || payoutsLoading || rateLoading;
        ...
        <div className="space-y-3">
            ...
            <div className="bg-white rounded-xl p-4">
                ...
                {rateLoading ? (
                    ...
                    <>
                        <p className="text-xl font-bold text-gray-900">Rp {ratePerCup.toLocaleString("id-ID")} / cup</p>
                        {effectiveDate && (
                            <p className="text-sm text-gray-400 mt-0.5">Effective {format(parseISO(effectiveDate), "d MMM yyyy")}</p>
                        )}
                    </>
                )}
            </div>
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    {payout ? (
                                        <p className="text-sm text-gray-500">
                                            ...
                                            {payout.reimbursementsTotal > 0 && (
                                                <span className="text-gray-400"> · {payout.cupsPayTotal.toLocaleString("id-ID")} cups + Rp {payout.reimbursementsTotal.toLocaleString("id-ID")} claims</span>
                                            )}
                                        </p>
                                    ) : (
