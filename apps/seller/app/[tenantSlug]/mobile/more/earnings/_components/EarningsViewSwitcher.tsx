...
export function EarningsViewSwitcher({
    ...
    return (
        <div className="grid grid-cols-2 bg-slate-200 rounded-xl p-1 w-full">
            {tabs.map((tab) => (
                <button
                    key={tab.value}
                    onClick={() => onChange(tab.value)}
                    className={`py-1.5 rounded-lg text-sm font-semibold text-center transition-all duration-200 ${
                        view === tab.value ? "bg-white text-slate-950 shadow-sm" : "text-slate-600"
                    }`}
                >
                    {tab.label}
                </button>
