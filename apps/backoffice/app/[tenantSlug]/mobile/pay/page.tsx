...
import { HandCoins, IdCard, Coins, Percent, ReceiptText, CalendarClock, ChevronRight } from "lucide-react";
...
function CurrentPeriodTotals() {
        ...
        () =>
            payouts
                ...
                .reduce(
                    ...
                    { commissions: 0, claims: 0, totalPay: 0, approved: 0, reviewed: 0 },
                ),
    ...
    return (
        <div className="bg-white rounded-2xl p-2 space-y-2">
            {/* The date is the period, and the period is the payouts list — so
                the line that names it is the way in. */}
            <button
                onClick={() => navigation.push(url("/mobile/pay/payouts"))}
                className="w-full flex items-center gap-2.5 text-left active:opacity-60"
            >
                ...
                <ChevronRight size={20} className="text-gray-400 shrink-0 ml-auto" />
            </button>
            ...
            <div className="bg-slate-50 rounded-xl p-2 grid grid-cols-4 gap-2">
                {/* Money is warm and the warmth deepens towards what is owed:
                    amber and yellow are the two halves, red is the sum. The
                    money leads the grid and the count trails it — what the
                    period costs is the question, and how much of it has been
                    looked at is the footnote. */}
                ...
                <div className={`p-2 rounded-lg col-span-2 ${missingApproval ? "bg-amber-100" : allApproved ? "bg-green-100" : "bg-slate-200"}`}>
                    ...
                    >
                        {totals.approved}/{totals.reviewed}
                    </p>
                </div>
            </div>
        </div>
    ...
    return (
        <div className="space-y-4">
            ...
            <SettingsGroup title="Configuration">
                ...
                />
                {/* Last because it is set once and left alone — the rows above
                    are the ones an admin comes back to. */}
                <SettingsRow
