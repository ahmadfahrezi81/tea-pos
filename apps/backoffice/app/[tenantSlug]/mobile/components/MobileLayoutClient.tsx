...
export default function MobileLayoutClient({ children }: MobileLayoutClientProps) {
    ...
    const lastRootTabRef = useRef<string>(url("/mobile/dashboard"));
    ...
    useEffect(() => {
        router.prefetch(url("/mobile/dashboard"));
        router.prefetch(url("/mobile/pay"));
    ...
    const currentTitle = currentRoute?.title ?? "Backoffice";
