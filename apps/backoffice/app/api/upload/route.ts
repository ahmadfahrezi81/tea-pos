...
const ALLOWED_BUCKETS = ["payroll-proofs", "reimbursements"] as const;
type AllowedBucket = (typeof ALLOWED_BUCKETS)[number];
const ALLOWED_TYPES = ["image/jpeg", "image/jpg", "image/webp"];
const MAX_SIZE_MB = 5;

function isSafeSubPath(s: string) {
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!bucket || !(ALLOWED_BUCKETS as readonly string[]).includes(bucket)) return badRequest("Invalid or missing bucket");
        if (!isSafeSubPath(rawSubPath)) return badRequest("Invalid subPath");
        ...
        const storagePath = rawSubPath ? `${tenantId}/${rawSubPath}/${filename}` : `${tenantId}/${filename}`;
