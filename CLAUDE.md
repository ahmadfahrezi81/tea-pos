...
- `routes.ts` — `RouteConfig` type. Every field describes a *layout capability* (`inlineHeader`, `headerAction`, `titleAccessory`, `footerCtaKey`, `preserveScroll`, `scrollPaddingBottom`, `refreshable`), never a specific screen. `parent: null` marks a root tab; `"lastRootTab"` returns to whichever tab the user came from.
...
- `navProgress.ts` + `nav-progress.css` — the navigation progress bar, deliberately **outside React**: the module writes `data-state` on one always-mounted node and the CSS animates only `transform`/`opacity`, so it keeps moving while the main thread commits a route. Never drive it with React state. Its CSS animations are declared **paused** and exist from mount; only `navProgress` plays, pauses and rewinds them — never create or switch one on by a state rule, or a slow Android freezes it (task 070). The bar ends after frames flow unbroken for a moment, not when data arrives. Both apps `@import` the CSS from `globals.css`. Each app's `MobileLayoutClient` wraps page content in an `SWRConfig` whose `trackFirstLoad` middleware (`lib/utils/`) holds the bar until the page's first-load requests settle. Pull to refresh lives in the shell too — it slides the scroll container down to open a gap with a spinner, using passive touch listeners and direct style writes, clears the transform at rest, is opt-in per route with `refreshable` (which also turns off native bounce there), refetching through the app's `onRefresh` (`mutate(() => true)`, never a reload). See task 065.
