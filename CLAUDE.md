...
- `navProgress.ts` + `nav-progress.css` — the navigation progress bar, deliberately **outside React**: the module writes `data-state` on one always-mounted node and the CSS animates only `transform`/`opacity`, so it keeps moving while the main thread commits a route. Never drive it with React state. Both apps `@import` the CSS from `globals.css`. Each app's `MobileLayoutClient` wraps page content in an `SWRConfig` whose `trackFirstLoad` middleware (`lib/utils/`) holds the bar until the page's first-load requests settle. Pull to refresh lives in the shell too — it slides the scroll container down to open a gap with a spinner, using passive touch listeners and direct style writes, clears the transform at rest, is opt-in per route with `refreshable` (which also turns off native bounce there), refetching through the app's `onRefresh` (`mutate(() => true)`, never a reload). See task 065.
