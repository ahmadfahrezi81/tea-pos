...
//             // Fetch photos for all summaries
//             const { data: photos, error: photosError } = await supabase
//                 .from("daily_summary_photos")
//                 .select(
//                     "id, type, url, created_at, daily_summary_id, expense_id",
//                 )
//                 .in("daily_summary_id", summaryIds)
//                 .eq("tenant_id", currentTenantId)
//                 .order("created_at", { ascending: true });
...
//         // ─── Build final summaries with expenses + photos attached ─────
...
//                 // Also lock in closing cash breakdown if provided
//                 if (closingCashBreakdown !== undefined) {
//                     updates.closing_cash_breakdown = closingCashBreakdown;
//                 }
...
export async function GET(request: NextRequest) {
    try {
        ...
        // ─── Fetch expenses + photos for all summaries ─────────────────
        const summaryIds = summaryList.map((s) => s.id);
        const expensesByDate: Record<string, ExpenseRow[]> = {};
        const expensesBySummaryId: Record<string, ExpenseRow[]> = {};
        const photosBySummaryId: Record<string, PhotoRow[]> = {};
