...
//         if (path.endsWith("/mobile/profile/map")) return null; // was "New Location"
...
export default function MobileLayoutClient({
    ...
    // Pages where the layout header is hidden entirely — the page manages its own chrome
    const isFullscreenPage = currentPath.endsWith("/mobile/profile/map");

    // Pages where the header is compact: back + title on one row (no stacked title)
    const isInlineHeader = currentPath.endsWith(
        "/mobile/analytics/daily/close",
    );

    // Scroll container top padding depends on header style
